...
**Phiên bản:** `1.1.0` (theo `manifest.json`)
...
- Tải nhạc từ video TikTok:
  - Hiển thị nút `♫ Tải nhạc` trực tiếp trên video khi rê chuột.
  - Tự động tải file MP3 khi lấy được link nhạc trực tiếp.
  - Tự mở trang chuyển đổi MP3 (fallback) nếu không lấy được link nhạc trực tiếp.
- Tải video (không watermark):
  - Hiển thị nút `⬇ Video (no logo)` cạnh nút tải nhạc trên các khối video feed/modal.
  - Lấy link phát video qua API bên thứ ba (tiker.io), tải file MP4 qua quyền `downloads` của Chrome.
  - Nếu không lấy được link video, extension sẽ báo lỗi (không có fallback tự mở trang như luồng MP3).
...
- Host permissions:
  ...
  - `https://tiktok.com/*`
  - `https://tiker.io/*` (dùng cho tải nhạc / tải video không logo)
...
6. Với video TikTok trên feed, rê chuột vào video và bấm **♫ Tải nhạc** để tải MP3.
7. Bấm **⬇ Video (no logo)** trên cùng khối video để tải file MP4 (phiên bản không watermark khi API trả về link hợp lệ).
8. Nếu extension không lấy được link nhạc trực tiếp, hệ thống sẽ tự mở trang fallback để bạn tiếp tục chuyển đổi/tải MP3.
